...
self.onmessage = (event) => {
    ...
    // DLX計算
    startPacking_dlx_worker(problem);
};

function startPacking_dlx_worker(
    problem: PackingProblem
) {
    // ...packing_ver_dlx.tsのstartPacking_dlxの内容をほぼそのまま...
    // onSolved, onFinished の代わりに postMessage で結果を返す
    function onSolved(solution: PackingSolution) {
